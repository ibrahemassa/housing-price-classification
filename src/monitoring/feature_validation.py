    ...
    def _calculate_reference_stats(self) -> dict:
        ...
        for col in self.reference_data.columns:
            ...
            if pd.api.types.is_numeric_dtype(series):
                ...
                col_stats["most_frequent"] = (
                    series.mode().iloc[0] if len(series.mode()) > 0 else None
                )
                col_stats["most_frequent_pct"] = (
                    float((series == col_stats["most_frequent"]).sum() / len(series))
                    if col_stats["most_frequent"] is not None
                    else 0.0
                )
                col_stats["missing_count"] = int(series.isna().sum())
                col_stats["missing_pct"] = float(series.isna().sum() / len(series))
    ...
    def validate_production_data(
        ...
        for col, ref_stats in self.reference_stats.items():
            ...
            if ref_stats["type"] == "numerical":
                ...
                outliers = (
                    (prod_series < lower_bound) | (prod_series > upper_bound)
                ).sum()
            ...
            validations_dict[col] = col_validation
