...
def get_mlflow_metrics():
    ...
    try:
        ...
        metrics = {
            ...
            "address_cardinality_ratio": latest_run.get("metrics.address_cardinality_ratio", None),
            "prediction_kl_drift": latest_run.get("metrics.prediction_kl_drift", None),
            "total_alerts": latest_run.get("metrics.total_alerts", None),
            "alerts": latest_run.get("metrics.total_alerts", None),  # Backward compatibility
            "drift_severity": latest_run.get("metrics.drift_severity", None),
            "feature_validation_alerts": latest_run.get("metrics.feature_validation_alerts", None),
        ...
        numeric_metrics = ["district_psi", "address_cardinality_ratio", "prediction_kl_drift", 
                          "total_alerts", "alerts", "drift_severity", "feature_validation_alerts"]
...
def calculate_feature_statistics(ref_df, prod_df):
    ...
    for col in ref_df.columns:
        ...
        if pd.api.types.is_numeric_dtype(ref_series):
            ...
            stat_dict["cardinality_ratio"] = stat_dict["prod_unique"] / max(stat_dict["ref_unique"], 1)
...
def main():
    ...
    page = st.sidebar.radio(
        "Select Page",
        ["Overview", "Drift Monitoring", "Feature Statistics", "Performance Metrics", "Production Data", "Alerts"],
    )
    ...
    if page == "Overview":
        if ref_data is None:
            st.warning("⚠️ Reference data not found. Please ensure reference.parquet exists.")
        if prod_inputs is None or prod_preds is None:
            st.warning("⚠️ Production data not found. Waiting for production predictions...")
            st.info("The dashboard will update automatically once production data is available.")
        if ref_data is not None:
            show_overview(ref_data, prod_inputs, prod_preds, mlflow_metrics, model_metrics_df, prefect_runs)
        ...
        if ref_data is None or prod_inputs is None:
            st.warning("⚠️ Reference and production data required for feature statistics.")
        else:
...
def show_overview(ref_data, prod_inputs, prod_preds, mlflow_metrics, model_metrics_df, prefect_runs=None):
    ...
    with col2:
        if mlflow_metrics:
            ...
            alerts = safe_get_metric(mlflow_metrics, "total_alerts") or safe_get_metric(mlflow_metrics, "alerts", 0)
            alerts = int(alerts)
    ...
    if "timestamp" in prod_preds.columns:
        preds_by_time = prod_preds.groupby(
            prod_preds["timestamp"].dt.floor("H")
        ...
        if model_metrics_df is not None and not model_metrics_df.empty:
            ...
            if not df_clean.empty:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name="Accuracy",
                    mode="lines+markers",
                    line=dict(color=CATPPUCCIN_COLORS["blue"], width=2),
                    marker=dict(color=CATPPUCCIN_COLORS["blue"]),
                ))
                fig.add_trace(go.Scatter(
                    x=df_clean.index,
                    y=df_clean["macro_f1"],
                    name="Macro F1",
                    mode="lines+markers",
                    line=dict(color=CATPPUCCIN_COLORS["green"], width=2),
                    marker=dict(color=CATPPUCCIN_COLORS["green"]),
                ))
                fig.update_layout(
    ...
    if prefect_runs:
        ...
        st.info("Prefect client not available. Ensure Prefect server is running and configured.")
...
def show_drift_monitoring(ref_data, prod_inputs, prod_preds, mlflow_metrics):
    ...
    if mlflow_metrics:
        ...
        with col2:
            ...
            status = "⚠️ High Growth" if card_ratio > CARDINALITY_THRESHOLD else "✅ Normal"
            st.metric(
        ...
        with col3:
            ...
            status = "⚠️ Drift Detected" if pred_drift > PRED_DRIFT_THRESHOLD else "✅ Stable"
            st.metric(
                "Prediction KL Drift",
                f"{pred_drift:.4f}",
                delta=status,
    ...
    if "price_category" in ref_data.columns:
        ref_pred_dist = ref_data["price_category"].value_counts(normalize=True).sort_index()
    else:
...
def show_feature_statistics(ref_data, prod_inputs):
    ...
    if "drift_detected" in display_df.columns:
        display_df["drift_detected"] = display_df["drift_detected"].map({True: "⚠️ Yes", False: "✅ No"})
    ...
    with col1:
        ...
        if "drift_detected" in feature_row:
            status = "⚠️ Drift Detected" if feature_row["drift_detected"] else "✅ No Drift"
    ...
    with col2:
        if feature_row["type"] == "numerical":
            ...
            if "cardinality_ratio" in feature_row:
                st.metric("Cardinality Ratio", f"{feature_row['cardinality_ratio']:.2f}")
...
def show_performance_metrics(model_metrics_df, ref_data, prod_preds):
    ...
    with col1:
        st.metric("Accuracy", f"{latest['accuracy']:.4f}" if pd.notna(latest.get('accuracy')) else "N/A")
    with col2:
        st.metric("Macro Precision", f"{latest['macro_precision']:.4f}" if pd.notna(latest.get('macro_precision')) else "N/A")
    with col3:
        st.metric("Macro Recall", f"{latest['macro_recall']:.4f}" if pd.notna(latest.get('macro_recall')) else "N/A")
    with col4:
        st.metric("Macro F1", f"{latest['macro_f1']:.4f}" if pd.notna(latest.get('macro_f1')) else "N/A")
    with col5:
    ...
    with col1:
        st.metric("Weighted Precision", f"{latest['weighted_precision']:.4f}" if pd.notna(latest.get('weighted_precision')) else "N/A")
    with col2:
        st.metric("Weighted Recall", f"{latest['weighted_recall']:.4f}" if pd.notna(latest.get('weighted_recall')) else "N/A")
    with col3:
        st.metric("Weighted F1", f"{latest['weighted_f1']:.4f}" if pd.notna(latest.get('weighted_f1')) else "N/A")
    ...
    with col2:
        if latest.get("start_time"):
            st.metric("Last Training", latest["start_time"].strftime("%Y-%m-%d") if hasattr(latest["start_time"], "strftime") else "N/A")
    ...
    display_cols = [
        "run_id", "model", "accuracy", "macro_precision", "macro_recall",
        "macro_f1", "weighted_f1", "roc_auc_macro"
    ]
    ...
    if "price_category" in ref_data.columns:
        ...
        st.info("💡 For continuous evaluation, ground truth labels are needed to calculate production accuracy.")
        ...
        with col2:
            ...
            if "timestamp" in prod_preds.columns:
                time_span = (prod_preds["timestamp"].max() - prod_preds["timestamp"].min())
                st.metric("Time Span", f"{time_span.days} days" if hasattr(time_span, "days") else "N/A")
...
def show_production_data(prod_inputs, prod_preds):
    ...
    with col3:
        if "timestamp" in prod_inputs.columns:
            ...
            st.metric("Latest Input", latest.strftime("%Y-%m-%d %H:%M") if hasattr(latest, "strftime") else "N/A")
    ...
    feature_to_plot = st.selectbox("Select Feature to Visualize", prod_inputs.columns.tolist())
...
def show_alerts(ref_data, prod_inputs, prod_preds, mlflow_metrics):
    ...
    if not feature_stats.empty:
        drifted_features = feature_stats[feature_stats.get("drift_detected", False) == True]
        if not drifted_features.empty:
            for _, row in drifted_features.iterrows():
                alerts.append({
                    "type": "🟠 Alert",
                    "metric": f"Feature: {row['feature']}",
                    "value": f"PSI: {row.get('psi', 'N/A'):.4f}" if 'psi' in row else "N/A",
                    "threshold": "N/A",
    ...
    status_items = [
        ...
        ("Production Inputs", "✅ Available" if prod_inputs is not None else "❌ Missing"),
        ("Production Predictions", "✅ Available" if prod_preds is not None else "❌ Missing"),
