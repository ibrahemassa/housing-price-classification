    ...
    for i in range(len(keys)):
        ...
        if expected_pct > eps:
            psi += (actual_pct - expected_pct) * np.log(
                (actual_pct + eps) / (expected_pct + eps)
            )
        elif actual_pct > eps:
    ...
    with mlflow.start_run(run_name="drift-check"):
        ...
        if CAT_FEATURE in ref.columns and CAT_FEATURE in prod_inputs.columns:
            ...
            if d_psi > PSI_THRESHOLD:
                alerts += 1
                logging.warning(
                    f"District PSI ({d_psi:.4f}) exceeds threshold ({PSI_THRESHOLD})"
                )
        else:
            logging.warning(
                f"Column '{CAT_FEATURE}' not found in reference or production data"
            )
            d_psi = 0.0
            drift_metrics["district_psi"] = d_psi

        if (
            HIGH_CARD_FEATURE in ref.columns
            and HIGH_CARD_FEATURE in prod_inputs.columns
        ):
            ...
            if card_ratio > CARDINALITY_THRESHOLD:
                alerts += 1
                logging.warning(
                    f"Address cardinality ratio ({card_ratio:.2f}) exceeds threshold ({CARDINALITY_THRESHOLD})"
                )
        else:
            logging.warning(
                f"Column '{HIGH_CARD_FEATURE}' not found in reference or production data"
            )
            card_ratio = 1.0
            drift_metrics["address_cardinality_ratio"] = card_ratio
        ...
        if "price_category" in ref.columns:
            ...
            logging.warning(
                "No 'price_category' or 'target' column found in reference data"
            )
        ...
        if ref_pred_col is not None and "prediction" in prod_preds.columns:
            ...
            if pred_drift > PRED_DRIFT_THRESHOLD:
                alerts += 1
                logging.warning(
                    f"Prediction KL divergence ({pred_drift:.4f}) exceeds threshold ({PRED_DRIFT_THRESHOLD})"
                )
        else:
            logging.warning(
                "Cannot calculate prediction drift - missing reference or production predictions"
            )
            pred_drift = 0.0
            drift_metrics["prediction_kl_drift"] = pred_drift
        ...
        try:
            ...
            mlflow.log_metric(
                "feature_validation_passed", 1 if validation_results["passed"] else 0
            )
            mlflow.log_metric(
                "feature_validation_alerts", len(validation_results["alerts"])
            )
            mlflow.log_metric(
                "feature_validation_warnings", len(validation_results["warnings"])
            )
...
""",
        ...
        try:
            ...
            ref_pred_for_plot = (
                ref_pred_col
                if ref_pred_col is not None
                else ref.get("target", pd.Series())
            )
            if len(ref_pred_for_plot) > 0:
                ...
                logging.warning(
                    "Cannot create prediction plot - missing reference predictions"
                )
