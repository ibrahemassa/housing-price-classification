...
FALLBACK_MODEL_PATH = "fallback_models/model.pkl"
...
def load_model(alias="production"):
    ...
    try:
        ...
        try:
            # Try primary local path first (volume mounted)
            if os.path.exists(LOCAL_MODEL_PATH):
                model = joblib.load(LOCAL_MODEL_PATH)
                print(f"Successfully loaded model from {LOCAL_MODEL_PATH}")
                return model
            # Try fallback path (baked into Docker image)
                ...
                raise RuntimeError(
                    ...
                    f"MLflow error: {error_msg[:200]}, Paths tried: {LOCAL_MODEL_PATH}, {FALLBACK_MODEL_PATH}"
                )
