...
class TestProductionLogger:
    ...
    def test_log_input_creates_new_file(self, temp_log_dir, monkeypatch):
        ...
        monkeypatch.setattr("src.utils.production_logger.INPUT_LOG", Path(temp_log_dir) / "inputs.parquet")
        ...
        assert Path(temp_log_dir) / "inputs.parquet" == Path(temp_log_dir) / "inputs.parquet"
    ...
    def test_log_input_appends_to_existing_file(self, temp_log_dir, monkeypatch):
        ...
        # Create initial file
        initial_data = pd.DataFrame([{"GrossSquareMeters": 50.0, "timestamp": datetime.utcnow()}])
    ...
    def test_log_prediction_creates_new_file(self, temp_log_dir, monkeypatch):
        ...
        monkeypatch.setattr("src.utils.production_logger.LOG_DIR", Path(temp_log_dir))
        monkeypatch.setattr("src.utils.production_logger.PRED_LOG", Path(temp_log_dir) / "predictions.parquet")
