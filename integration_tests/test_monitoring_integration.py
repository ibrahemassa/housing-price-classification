...
class TestMonitoringIntegration:
    ...
    def test_monitoring_flow_with_real_data(self, temp_monitoring_dir, reference_data, 
                                           production_data, production_predictions):
        ...
        drift = prediction_drift(reference_data["price_category"], production_predictions["prediction"])
    ...
    def test_monitoring_flow_integration(self, mock_subprocess, mock_artifact, 
                                        mock_pred_plot, mock_card_plot, mock_dist_plot,
                                        mock_read_parquet, mock_exists,
                                        reference_data, production_data, production_predictions):
    ...
    def test_plot_functions_integration(self, temp_monitoring_dir, reference_data, production_data):
        ...
        with patch("src.monitoring.plot.PLOT_DIR", Path(temp_monitoring_dir) / "data/monitoring/plots"):
            # Test categorical distribution plot
            path1 = plot_categorical_distribution(reference_data, production_data, "district")
